...
import { SudokuBoard, Cell } from '../../../shared/types.js';
...
export class SudokuGenerator {
  ...
  /**
   * Convert flat array to SudokuBoard with Cell objects
   * The sudoku library uses 0-8 for values, we convert to 1-9
   */
  private static arrayToBoard(arr: (number | null)[]): SudokuBoard {
    const cells: Cell[][] = [];
    ...
    for (let row = 0; row < 9; row++) {
      ...
      for (let col = 0; col < 9; col++) {
        ...
        cells[row][col] = {
          value: value !== null ? value + 1 : null, // Convert 0-8 to 1-9
          locked: value !== null, // Pre-filled cells are locked
          lockedBy: value !== null ? 'system' : null,
          revealed: false
        };
      }
    }
